...
import com.dusty.boring.mybatis.sql.common.cache.CacheKeyGenerator;
...
public class BadSqlValidateIntercepter implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        try {
            ...
            String cacheKey = CacheKeyGenerator.buildLocalCacheKey(sqlImage);
            
            //检查sql白名单（包括已知通过校验的sql及已知标记‘IgnoreSqlChecker’忽略的sql）
            if (enabledEnvsWhiteList(sqlValidatorProperties) && whiteSqlList.containsKey(cacheKey)) {
               return invocation.proceed();
            }
            
            //检查已知黑名单
            if (enabledEnvsBlackList(sqlValidatorProperties)) {
                SqlValidateResult result = blackSqlList.get(cacheKey);
            ...
            final SqlValidateResult validateResult = sqlValidateProvider.validateSqlWithResult(toExecSql, cacheKey, conn);
            if (Objects.nonNull(validateResult) && validateResult.getViolations().size() > 0) {
                ...
                /**
                 * {@link AbstractSqlValidateProvider#addBlackSql(String, SqlValidateResult)}
                 *
                 * if (enabledEnvsBlackList(sqlValidatorProperties)) {
                 *   blackSqlList.put(cacheKey, validateResult);
                 *  }
                 */
